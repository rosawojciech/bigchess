test_that("is_check_cpp returns correct values", {
  ...
  expect_true(is_check_cpp(position, 1)) # Black is in check
  ...
  position[8,6] <-  0 # Remove white bishop at f1
  expect_true(is_check_cpp(position, -1)) # White is in checkmate
})
